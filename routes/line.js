...
publicRouter.get('/:competitionid/score/:league/print', function (req, res, next) {
  const id = req.params.competitionid;
  const league = req.params.league;
  ...
  competitiondb.competition.findOne({
    ...
    if (err) {
      ...
      res.render('line_score_print', {id: id, user: req.user,league: league,num: num, get: req.query})
    }
  })
})
...
privateRouter.get('/check/:runid', async function (req, res, next) {
  ...
  let rule = await ruleDetector.getRuleFromLineRunId(id);
  res.render('line_check', {id: id, rule: rule})
})
