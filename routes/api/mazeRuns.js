  ...
  mazeRun
    ...
    .exec(function (err, dbRuns) {
      if (err) {
        ...
        for (let i = 0; i < dbRuns.length; i++) {
          const retScoreCals = scoreCalculator
            .calculateMazeScore(dbRuns[i])
            .split(',');

          mazeRun.findById(dbRuns[i]._id).exec(function (err, dbRun) {
...
function getLatestMazeRun(req, res) {
  ...
  const query = mazeRun.findOne(selection).sort('-updatedAt');
